...
module conv1_accel_icb #(
    ...
    input  wire                      i_icb_cmd_read,    // 1=read,0=write
    input  wire [ICB_RW_ADDR_W-1:0]  i_icb_cmd_addr,
    input  wire [ICB_RW_DATA_W-1:0]  i_icb_cmd_wdata,
    input  wire [3:0]                i_icb_cmd_wmask,

    output reg                       i_icb_rsp_valid,
    input  wire                      i_icb_rsp_ready,
    output reg  [ICB_RW_DATA_W-1:0]  i_icb_rsp_rdata
);

    // ===========================
    // 地址映射
    // ===========================
    localparam ADDR_CTRL      = 32'h0000_0000;   // start bit
    localparam ADDR_STATUS    = 32'h0000_0004;   // done bit
    ...
    // ICB 接收握手
    // ===========================
    assign i_icb_cmd_ready = 1'b1;  // always ready
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (i_icb_cmd_addr == ADDR_CTRL) begin
                start_reg <= i_icb_cmd_wdata[0];  // bit0=start
            end
