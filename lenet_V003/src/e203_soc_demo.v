module e203_soc_demo (
    ...
    // UART pins (txp: FPGA->PC, rxp: PC->FPGA)
    output         txp,
    input          rxp,
    
    // Extra UART TX output to PMID pin
    output         pmid_txp
);
    ...
    wire uart_clk;   // UART dedicated 24 MHz clock
    ...
    clk_unit clk_unit (
        ...
        .reset(erstn), 
        .clkin(clk_in), 
        .clkout_system(hfextclk), 
        .clkout_uart(uart_clk),  // UART 24 MHz clock
        .lock(pll_lock)
    );
...
e203_soc_top e203_soc_ins (
    ...
    // UART0 RX uses GPIO16 IOF0
    .io_pads_gpio_16_i_ival  (rxp),
...
// Drive UART TX pin from GPIO17 function output (IOF0)
assign txp = gpio_out[17];

// Drive PMID UART TX pin from the same GPIO17 signal
assign pmid_txp = gpio_out[17];
